...
namespace
{
  ...
  bool IsLivoxTeleDataName(const std::string &data_name)
  {
    std::string normalized = data_name;
    ...
    return normalized == "livox_tele" ||
           normalized == "Livox_tele" ||
           normalized == "tele";
  }


  bool ReadLivoxCustomMsg(const std::string &file_name, livox_ros_driver2::msg::CustomMsg &msg)
  {
    ...
    while (true) {
      ...
      if (!file.read(reinterpret_cast<char *>(&point.x), sizeof(float))) break;
      if (!file.read(reinterpret_cast<char *>(&point.y), sizeof(float))) break;
      if (!file.read(reinterpret_cast<char *>(&point.z), sizeof(float))) break;
      if (!file.read(reinterpret_cast<char *>(&point.reflectivity), sizeof(uint8_t))) break;
      if (!file.read(reinterpret_cast<char *>(&point.tag), sizeof(uint8_t))) break;
      if (!file.read(reinterpret_cast<char *>(&point.line), sizeof(uint8_t))) break;
      if (!file.read(reinterpret_cast<char *>(&point.offset_time), sizeof(uint32_t))) break;
...
void ROSThread::ros_initialize(rclcpp::Node::SharedPtr node)
{
    ...
    livox_tele_pub_ = node_->create_publisher<livox_ros_driver2::msg::CustomMsg>("/livox_tele/points", 1000000);
...
void ROSThread::Ready()
{
  ...
  char data_name[50];
  data_stamp_.clear();
  while(fscanf(fp,"%ld,%49s\n",&stamp,data_name) == 2){
    // std::cout<<stamp<<","<<data_name<<std::endl;
    data_stamp_.insert( multimap<int64_t, string>::value_type(stamp, data_name));
  }

  // City02 keeps Livox Tele files and timestamps under sensor_data/.
  // Merge only Livox Tele rows from sensor_data/data_stamp.csv, so existing
  // stamp.csv playback for other sensors remains unchanged.
  FILE *tele_stamp_fp = fopen((data_folder_path_ + "/sensor_data/data_stamp.csv").c_str(), "r");
  if (tele_stamp_fp != nullptr) {
    int64_t tele_stamp;
    char tele_data_name[50];
    int added_tele_stamp_count = 0;
    while(fscanf(tele_stamp_fp, "%ld,%49s\n", &tele_stamp, tele_data_name) == 2){
      std::string tele_data_name_str(tele_data_name);
      if(!IsLivoxTeleDataName(tele_data_name_str)) {
        continue;
      }

      bool already_exists = false;
      auto range = data_stamp_.equal_range(tele_stamp);
      for(auto iter = range.first; iter != range.second; ++iter){
        if(IsLivoxTeleDataName(iter->second)){
          already_exists = true;
          break;
        }
      }
      ...
      if(!already_exists){
        data_stamp_.insert(multimap<int64_t, string>::value_type(tele_stamp, "livox_tele"));
        added_tele_stamp_count++;
      }
    }
    fclose(tele_stamp_fp);
    if(added_tele_stamp_count > 0){
      std::cout << "Livox_tele stamps added from sensor_data/data_stamp.csv: "
                << added_tele_stamp_count << std::endl;
    }
  }
  ...
  while(1){
    ...
    if(length == 8){
      ...
      // City02 xsens_imu.csv format:
      // stamp,qx,qy,qz,qw,gx,gy,gz,ax,ay,az
      // With the current fscanf variable order:
      // x,y,z   receive gx,gy,gz
      // g_x,g_y,g_z receive ax,ay,az
      imu_data.header.stamp = rclcpp::Time(stamp);
      imu_data.header.frame_id = "imu";
  ...
  cout << "IMU data are loaded: " << imu_data_.size() << endl;
  fclose(fp);
  ...
  livox_tele_folder_path_.clear();
  const std::vector<std::string> livox_tele_dir_candidates = {
    data_folder_path_ + "/LiDAR/Livox_tele",
    data_folder_path_ + "/LiDAR/Tele",
    data_folder_path_ + "/sensor_data/Livox_tele",
    data_folder_path_ + "/Livox_tele"
  };
  for (const auto &candidate : livox_tele_dir_candidates) {
    if (DirectoryExists(candidate)) {
      livox_tele_folder_path_ = candidate;
      break;
    }
  }
  if (!livox_tele_folder_path_.empty()) {
    GetDirList(livox_tele_folder_path_, livox_tele_file_list_);
  } else {
    std::cout << "Livox_tele directory was not found. Checked LiDAR/Livox_tele, LiDAR/Tele, sensor_data/Livox_tele, and Livox_tele." << std::endl;
  }
  ...
  for(auto iter = data_stamp_.begin() ; iter != data_stamp_.end() ; iter ++){
    ...
    if(iter->second.compare("inspva") == 0){
      ...
      imu_thread_.cv_.notify_all();
        ...
        avia_thread_.cv_.notify_all();
    }else if(IsLivoxTeleDataName(iter->second)){
...
//        cout << "Re-load right ouster from path" << endl;
        ...
        if(find(next(ouster_file_list_.begin(),max(0,previous_file_index-search_bound_)),ouster_file_list_.end(),to_string(data)+".bin") != ouster_file_list_.end()){
            ...
            while (true){
                ...
                if (!file.read((char*)(&point.x), sizeof(float))) break;
                if (!file.read((char*)(&point.y), sizeof(float))) break;
                if (!file.read((char*)(&point.z), sizeof(float))) break;
                if (!file.read((char*)(&point.intensity), sizeof(float))) break;
                if (!file.read((char*)(&ring_in), sizeof(uint16_t))) break;
                if (!file.read((char*)(&t_in), sizeof(uint32_t))) break;
  ...
  while(1){
    ...
    while(!avia_thread_.data_queue_.empty()){
      ...
      if(to_string(data) + ".bin" == avia_next_.first){
        ...
        avia_next_.second.header.frame_id = "livox_avia";
        if(save_flag_ == true && process_flag_ == true){
          std::lock_guard<std::mutex> lock(bag_mutex_);
...
void ROSThread::LivoxTeleThread()
{
  ...
  while(1){
    ...
    while(!livox_tele_thread_.data_queue_.empty()){
      ...
      // publish cached next scan when available
      if(to_string(data) + ".bin" == livox_tele_next_.first){
        ...
        if(save_flag_ == true && process_flag_ == true){
          std::lock_guard<std::mutex> lock(bag_mutex_);
          bag_.write(livox_tele_next_.second, "/livox_tele/points", rclcpp::Time(data));
        }
        livox_tele_pub_->publish(livox_tele_next_.second);

      }else{
        livox_ros_driver2::msg::CustomMsg tele_msg;
        string current_file_name = livox_tele_folder_path_ + "/" + to_string(data) + ".bin";
        auto search_begin = next(livox_tele_file_list_.begin(), max(0, previous_file_index - search_bound_));
        auto found = find(search_begin, livox_tele_file_list_.end(), to_string(data) + ".bin");
        if(found != livox_tele_file_list_.end()){
          if(ReadLivoxCustomMsg(current_file_name, tele_msg)){
            tele_msg.header.stamp = rclcpp::Time(data);
            tele_msg.header.frame_id = "livox_tele";
            if(save_flag_ == true && process_flag_ == true){
              std::lock_guard<std::mutex> lock(bag_mutex_);
              bag_.write(tele_msg, "/livox_tele/points", rclcpp::Time(data));
            }
            livox_tele_pub_->publish(tele_msg);
          }
        }
        previous_file_index = 0;
      }
      ...
      // preload next scan
      if(!livox_tele_file_list_.empty()){
        auto search_begin = next(livox_tele_file_list_.begin(), max(0, previous_file_index - search_bound_));
        auto found = find(search_begin, livox_tele_file_list_.end(), to_string(data) + ".bin");
        if(found != livox_tele_file_list_.end()){
          current_file_index = found - livox_tele_file_list_.begin();
          if(current_file_index + 1 < static_cast<int>(livox_tele_file_list_.size())){
            livox_ros_driver2::msg::CustomMsg tele_msg;
            string next_file_name = livox_tele_folder_path_ + "/" + livox_tele_file_list_[current_file_index + 1];
            if(ReadLivoxCustomMsg(next_file_name, tele_msg)){
              tele_msg.header.stamp = rclcpp::Time(data);
              tele_msg.header.frame_id = "livox_tele";
              livox_tele_next_ = make_pair(livox_tele_file_list_[current_file_index + 1], tele_msg);
            }
          }
          previous_file_index = current_file_index;
        }
